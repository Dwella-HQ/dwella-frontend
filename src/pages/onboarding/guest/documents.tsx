...
const GuestOnboardingKycPage: NextPageWithLayout = () => {
  const router = useRouter();
  const { showToast } = useToast();
  const { user } = useUser();
  const [kyc, setKyc] = React.useState<GuestOnboardingKyc>(emptyGuestKyc);
  ...
  React.useEffect(() => {
    ...
    const details = sessionStorage.getItem(GUEST_ONBOARDING_KEYS.details);
    if (!details) {
      void router.replace("/onboarding/guest/details");
      return;
    }
    const stored = readJsonSession<GuestOnboardingKyc>(GUEST_ONBOARDING_KEYS.kyc);
    if (stored) setKyc({ ...emptyGuestKyc, ...stored });
  }, [router]);
  ...
  const persistKyc = React.useCallback((next: GuestOnboardingKyc) => {
    setKyc(next);
    sessionStorage.setItem(GUEST_ONBOARDING_KEYS.kyc, JSON.stringify(next));
  }, []);
  ...
  const handleUpload = React.useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (result.success) {
        persistKyc({
          ...kyc,
          governmentIdDocumentId: result.data.id,
  ...
  const handleContinue = React.useCallback(async () => {
    ...
    sessionStorage.setItem(GUEST_ONBOARDING_KEYS.kyc, JSON.stringify(kyc));

    const details =
      readJsonSession<GuestOnboardingDetails>(GUEST_ONBOARDING_KEYS.details) ??
      emptyGuestDetails;
    const userId =
      (user?.id != null ? String(user.id) : "") ||
      sessionStorage.getItem(GUEST_ONBOARDING_KEYS.userId) ||
      "";

    if (userId && (kyc.idType || kyc.idNumber.trim() || kyc.governmentIdDocumentId)) {
      const patch = await updateUser(userId, {
    ...
    await router.push("/onboarding/guest/complete");
    setIsSubmitting(false);
  }, [kyc, router, showToast, user?.id]);

  return (
    <>
      <Head>
        <title>Dwelliva · Guest KYC</title>
      </Head>
      ...
      <div className="mx-auto w-full max-w-4xl">
        <nav className="relative mb-6 flex flex-col items-center justify-between gap-4 sm:mb-8 sm:flex-row sm:gap-0">
          ...
          <div className="w-full sm:absolute sm:left-1/2 sm:w-auto sm:-translate-x-1/2">
            <SignUpProgress currentStep={2} steps={guestFlowSteps} />
          </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-8 shadow-sm">
          ...
          <div className="mt-6 flex items-center justify-between border-t border-gray-200 pt-6">
            <button
              type="button"
              onClick={() => router.push("/onboarding/guest/details")}
...
GuestOnboardingKycPage.getLayout = (page) => (
  <AuthLayout showImage={false}>{page}</AuthLayout>
);

export default GuestOnboardingKycPage;
