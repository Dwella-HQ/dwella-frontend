...
const GuestOnboardingCompletePage: NextPageWithLayout = () => {
  ...
  const handleDone = React.useCallback(async () => {
    ...
    markGuestOnboardingComplete();
    // Post-login guest screens — guest dashboard.
      ...
      <div className="mx-auto w-full max-w-4xl">
        <nav className="relative mb-6 flex flex-col items-center justify-between gap-4 sm:mb-8 sm:flex-row sm:gap-0">
          ...
          <div className="w-full sm:absolute sm:left-1/2 sm:w-auto sm:-translate-x-1/2">
            <SignUpProgress currentStep={3} steps={guestFlowSteps} />
          </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="mb-8 text-sm text-gray-600">
            Now let&apos;s get the best house for you
          </p>

          <div className="flex items-center justify-between border-t border-gray-200 pt-6">
            <button
              type="button"
              onClick={() => router.push("/onboarding/guest/documents")}
...
GuestOnboardingCompletePage.getLayout = (page) => (
  <AuthLayout showImage={false}>{page}</AuthLayout>
);

export default GuestOnboardingCompletePage;
