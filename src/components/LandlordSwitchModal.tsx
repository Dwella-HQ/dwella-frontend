  ...
  React.useEffect(() => {
    ...
    const fetchManagers = async () => {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          "[LandlordSwitchModal] landlord accounts with live totals:",
          enriched,
        );
      }
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-[100] max-h-[90vh] w-full max-w-5xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-xl focus:outline-none">
          <motion.div
