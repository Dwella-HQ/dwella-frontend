...
export const getProperties = async (): Promise<GetPropertiesResult> => {
  const result = await apiGet<PropertiesResponseDTO>("/property");
  ...
  try {
    ...
    console.error("Get properties schema validation error:", parseError);
    return {
      success: false,
